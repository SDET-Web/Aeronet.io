...
class Model_auth extends CI_Model
{
    ...
    function register_pilot($ID = '')
    {
        ...
        if ($this->input->post('action') == 'register_pilot_step_1') {
            ...
            if (!$this->form_validation->run() == FALSE) {
                ...
                if ($pilots_list->num_rows() > 0) {
                    ...
                    push_message('We counldn\'t find any profile with this name. Please try again or you can signup manually here!', 'ERRO');
                    redirect('register/pilot/phone_check/');
                }
            }
          ...
          //  } elseif ($this->input->post('type') == 'P' && $this->input->post('expired') == 'y') {
           ...
           // } elseif (($this->input->post('type') == 'P' || $this->input->post('type') == '') && $this->input->post('norequire') != 'n') {
            //} //elseif ($this->input->post('phone')<> '') {
             if ($this->input->post('phone')<> '' && $this->input->post('type') <> '') {
                 $pilot = $this->db->from('directory_pilot')->where('unique_id', $ID)->get()->row_array();
                // Phone Number Validation
                $this->form_validation->set_rules('phone', 'Phone Number ', 'required|regex_match[/^\d{1,14}$/]',
                ...
                if (!$this->form_validation->run() == FALSE) {
                    ...
                    $msg = "Hi {$fullName}, \n {$code} is your AeroNet Verification Code Thanks for signing up.";
		...
		$twilio = new Twilio\Rest\Client($sid, $token);
		 try{
			$message = $twilio->messages 
			  ->create($this->input->post('phone'), // to 
				   array( 
				       "from" => "+13392202376",       
				       "body" => $msg,
				   ) 
		    ...
		    push_message("Please enter a valid phone number.", 'error');
            ...
            //} elseif ($this->input->post('type') == 'P' && $this->input->post('expired') == 'y') {
              //  $id = $this->Model_user->insert_pilot_from_id($pilot, $ID);
                //redirect('register/pilot/contact/' . $id);
            //} elseif (($this->input->post('type') == 'P' || $this->input->post('type') <> '') ) {
                if (($this->input->post('type') == 'P' || $this->input->post('type') <> '') ) {
            ...
            $this->form_validation->set_rules('first_name', 'First Name', 'required', "enter email address");
            $this->form_validation->set_rules('last_name', 'Last Name', 'required');
     ...
     // Code to signup with only phone verification   
    function register_pilot_new($ID = '')
    { 
        if($this->input->post('unique_id')<>'' && $this->input->post('phone')<> '') {
         
            $this->form_validation->set_rules('cnumber', 'Certificate Number ', 'required|regex_match[/^\d{7}$/]',
                    array('regex_match' => 'Please provide a valid Certificate Number.')
                ); 
            ...
            $this->form_validation->set_rules('phone', 'Phone Number ', 'required|regex_match[/^\d{1,14}$/]',
                ...
                if (!$this->form_validation->run() == FALSE) {                   
                   ...
                   $fullName = $pilot['first_name']." ".$pilot['last_name'];
                    ...
                    $sid    = "AC565c10c7ad46dd5d5904339d317c75ac"; 
		...
		$twilio = new Twilio\Rest\Client($sid, $token);
		 try{
			$message = $twilio->messages 
             ...
             redirect('register/pilot/verify-phone-check/' . $id);
		}catch(Exception $e){
		    push_message("Please enter a valid US phone number.", 'error');
		    redirect('register/pilot/phone_check/');
		}
             ...
             $pilot = $this->db->from('user_temp')->where('id', $ID)->get()->row_array();
                if (($this->input->post('type') == 'P' || $this->input->post('type') <> '') ) {
                $this->form_validation->set_rules('verification_code', 'Verification Code*', 'required');
                if (!$this->form_validation->run() == FALSE) {
                    
                    if ($pilot['verification_code'] == $this->input->post('verification_code')) {
                        redirect('register/pilot/signup-new/' . 'UID0000');
                    } else {
                        $count = $this->input->post('signup_try');
                        $count = $count + 1;
                        return array('count' => $count, 'msg' => 'Entered verification code is not correct, please try again.');
            ...
            $this->form_validation->set_rules('email', 'Email Address', 'required|valid_email|is_unique[user.user_email]');
            $this->form_validation->set_rules('password', 'Password', 'required');
            $this->form_validation->set_rules('type', 'Type', 'required', "Select Type");
            $this->form_validation->set_rules('first_name', 'First Name', 'required', "enter email address");
            $this->form_validation->set_rules('last_name', 'Last Name', 'required');
        
         if ($this->form_validation->run() == FALSE) {
        $cid = mt_rand(111111,999999);
        $ID= "RA".$cid;
        $data['unique_id'] = $ID;     
        $data['first_name'] = $this->input->post('first_name');
        $data['last_name'] = $this->input->post('last_name');
        $data['type'] = strtolower($this->input->post('user_type'));
        
         $this->db->insert('directory_pilot', $data);
         $ID = $this->db->insert_id();
         
        $data2['user_fname'] = $this->input->post('first_name');
        $data2['user_lname'] = $this->input->post('last_name');
        $data2['user_email'] = $this->input->post('email');
        $data2['user_password'] = md5($this->input->post('password'));
        $data2['user_pilot_id'] = $ID;
        $data2['user_type'] = strtolower($this->input->post('user_type'));
        $data2['user_status'] = 'n';
        $data2['user_created'] = time();
        $data2['user_modified'] = time();
